...
async function debugAdminUser() {
    ...
    try {
        ...
        const allUsers = await db.query(`
            SELECT id, name, email, role, divisi, createdAt, updatedAt
            FROM user_ptrj
        ...
        const completeUsers = await db.query('SELECT * FROM user_ptrj');

        // Tampilkan tanpa password untuk security
        const safeUsers = completeUsers.map(u => ({
            ...u,
        ...
        if (adminQuery.length === 0) {
            ...
            for (const variation of variations) {
                const result = await db.query(`SELECT id, name, email, role FROM user_ptrj WHERE role = '${variation}'`);
                if (result.length > 0) {
            ...
            console.log('\n6. All existing roles in database:');
            const rolesQuery = await db.query('SELECT DISTINCT role FROM user_ptrj');
            console.table(rolesQuery);
        } else {
            console.log('✓ Found ADMIN users:');
            console.table(adminQuery.map(u => ({
                ...u,
                password: u.password ? '[HASHED]' : null
            })));
        }
